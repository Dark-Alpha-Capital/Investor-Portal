...
export async function deleteDealFolder(dealName: string) {
  ...
  try {
    ...
    if (error.response?.status === 404) {
      console.log(`Folder does not exist: /Deals/${folderName}. Skipping deletion.`);
      return `/Deals/${folderName}`;
    }
    
    // Handle auth failures (401, 403)
    if (error.response?.status === 401 || error.response?.status === 403) {
      throw new Error(`Authentication failed when deleting Nextcloud folder: ${error.message}`);
    }
