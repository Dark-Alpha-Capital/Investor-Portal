...
const InvestorsTableView = memo(function InvestorsTableView({
  investors,
  selectedIds,
  onToggleSelect,
  onToggleSelectAll,
  ...
  const allSelected =
    investors.length > 0 && selectedIds.size === investors.length;
  const someSelected =
...
// Card View Component
const InvestorsCardView = memo(function InvestorsCardView({
  investors,
  selectedIds,
  onToggleSelect,
}: {
...
export function InvestorsTableClient({
  ...
  const handleToggleSelectAll = useCallback(() => {
    setSelectedIds((prev) => {
      if (prev.size === investors.length) {
        return new Set();
      } else {
        return new Set(investors.map((i) => i.id));
      }
    });
  }, [investors]);

  // Generate page numbers to display - memoized to avoid recalculation
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <Select
            value={kycStatus}
            onValueChange={(value) =>
              updateParams({ investorsKyc: value }, true)
            }
          >
          ...
          <Select
            value={verified}
            onValueChange={(value) =>
              updateParams({ investorsVerified: value }, true)
            }
          >
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {pagination.totalCount} investor
