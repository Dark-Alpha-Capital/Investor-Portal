...
`;
  ...
  try {
    ...
    if (overwrite.toLowerCase() !== "y") {
      ...
      return "postgresql://postgres:postgres@localhost:5432/better_auth";
    }
  } catch {
  ...
  try {
    ...
    while (retries > 0) {
      try {
        await execAsync(
          "docker exec better_auth_postgres pg_isready -U postgres"
        );
...
async function getPostgresURL(): Promise<string> {
  ...
  if (dbChoice.toLowerCase() === "r") {
    ...
    return (
      url.trim() || "postgresql://user:password@localhost:5432/better_auth"
    );
  }
  log(
  ...
  return "postgresql://user:password@localhost:5432/better_auth";
}
...
async function runMigration(databaseUrl: string): Promise<boolean> {
  try {
    ...
    const migrationPath = path.join(
      process.cwd(),
      "lib/db/migrations/0000_gifted_deadpool.sql"
    );
...
async function main() {
  ...
  log("   pnpm dev", "cyan");
  log("3. Visit http://localhost:3000 to get started", "blue");
  if (databaseUrl && !databaseUrl.includes("user:password")) {
