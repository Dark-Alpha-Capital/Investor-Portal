...
export default async function InvestmentTypePage({
  ...
  return (
    <Suspense fallback={<InvestmentTypeSkeleton />}>
      <FetchInvestmentTypeWrapper params={params} />
    </Suspense>
  );
}

async function FetchInvestmentTypeWrapper({ params }: { params: Params }) {
  const { type: typeSlug } = await params;
  const investmentType = getInvestmentTypeBySlug(typeSlug);
  ...
  return (
    <>
      ...
      <Suspense fallback={<InvestmentTypeSkeleton />}>
        <InvestmentTypeContent typeSlug={typeSlug} />
      </Suspense>
    </>
  );
}

async function InvestmentTypeContent({ typeSlug }: { typeSlug: string }) {
  "use cache";
  cacheLife("days");
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      {/* Main Content */}
      <section className="mb-12">
        <h2 className="text-2xl font-bold mb-4">About {investmentType.name}</h2>
